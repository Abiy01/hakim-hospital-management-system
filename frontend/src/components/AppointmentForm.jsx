  ...
  const handleAppointment = async (e) => {
    ...
    try {
      ...
      if (error.response?.data?.message) {
        ...
        errorMessage = "Please login as a patient to book an appointment";
        setTimeout(() => {
          navigate("/login");
        }, 2000);
      } else if (error.response?.status === 403) {
        errorMessage = "You are not authorized to book appointments. Please login as a patient.";
        setTimeout(() => {
          navigate("/login");
        }, 2000);
      }
      ...
      <div className="container form-component appointment-form">
        ...
        {!isAuthenticated || user?.role !== "Patient" ? (
          ...
          <div>
            <input
              type="text"
              placeholder="NIC (National ID Card Number)"
              value={nic}
              onChange={(e) => setNic(e.target.value)}
              maxLength={13}
              title="Enter your National ID Card number"
            />
            <input
              type="date"
          ...
          <div>
            ...
            >
              <option value="">Select Doctor</option>
              {doctors.filter((doctor) => doctor.doctorDepartment === department).length === 0 ? (
